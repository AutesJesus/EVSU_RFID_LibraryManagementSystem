...
$otpPending = auth_patron_otp_pending();
if ($otpPending !== null && !$step_rfid) {
    $step_otp = true;
    $otp_masked_email = auth_mask_email((string) ($otpPending['email'] ?? ''));
}

$pdo = get_pdo();

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['patron_otp_resend'])) {
        $result = auth_resend_patron_email_otp();
        if ($result['ok']) {
            $msg = 'A new code was sent to your email.';
            if (ajax_is_requested()) {
                ajax_json_response(true, $msg, '', ['step' => 'email_otp', 'redirect' => 'login.php']);
            }
    ...
    } elseif (isset($_POST['patron_otp_verify'])) {
        $result = auth_verify_patron_email_otp(
            isset($_POST['otp_code']) ? (string) $_POST['otp_code'] : ''
        );
        if ($result['ok']) {
            if (ajax_is_requested()) {
    ...
    } elseif (isset($_POST['admin_rfid_verify'])) {
        ...
        if ($result['ok']) {
            ...
            if (isset($result['step']) && $result['step'] === 'email_otp') {
                if (ajax_is_requested()) {
                    ajax_json_response(true, 'Check your email for the sign-in code.', '', ['step' => 'email_otp', 'redirect' => 'login.php']);
                }
                header('Location: login.php');
                exit;
            }
...
<body class="login-page<?= $step_rfid ? ' auth-rfid-step' : '' ?><?= $step_otp ? ' auth-otp-step' : '' ?>">
    ...
    <div class="login-wrap">
        <main class="login-card" aria-labelledby="loginTitle">
            <header class="login-brand">
                ...
                <h1 id="loginTitle">Welcome back, user!</h1>
        ...
        (function () {
            ...
            f.addEventListener('submit', function (e) {
                ...
                ajaxPostForm(f).then(function (data) {
                    if (data.ok && (data.step === 'rfid' || data.step === 'email_otp')) {
    ...
    <?php if ($step_otp): ?>
    <div class="login-wrap">
        <main class="login-card" aria-labelledby="otpTitle">
            <header class="login-brand">
                <span class="login-brand-icon" aria-hidden="true">
                ...
                <p>We sent a 6-digit code to <strong><?= login_h($otp_masked_email) ?></strong></p>
            </header>

            <?php if ($otp_resent): ?>
                <p class="login-alert login-alert-ok" role="status">A new code was sent.</p>
            <?php endif; ?>
            <p id="ajaxOtpErr" class="login-alert" role="alert" hidden></p>
            <?php if ($error !== ''): ?>
                <p class="login-alert" role="alert"><?= login_h($error) ?></p>
            <?php endif; ?>

            <form method="post" action="" id="otpForm" novalidate>
                <input type="hidden" name="patron_otp_verify" value="1">
                <input type="hidden" name="__ajax" value="0" id="otpAjaxFlag">
            ...
            <form method="post" action="" class="login-otp-resend">
                <input type="hidden" name="patron_otp_resend" value="1">
                <button type="submit" class="login-link-btn">Resend code</button>
            </form>

            <p class="login-foot">
                <a href="login.php?cancel=1">Cancel and start over</a>
            </p>
        </main>
    </div>
    ...
    <script>
        (function () {
            ...
            var err = document.getElementById('ajaxOtpErr');
            var ajaxFlag = document.getElementById('otpAjaxFlag');
            ...
            f.addEventListener('submit', function (e) {
                ...
                ajaxPostForm(f).then(function (data) {
                    ...
                    if (err) {
                        err.textContent = data.message || data.error || 'Verification failed.';
                        err.hidden = false;
                    }
                }).catch(function () {
                    if (err) {
