...
# Sync built project to Hostinger over SSH (used by GitHub Actions and optional local deploy).
...
rsync -avz \
  ...
  --exclude 'deploy/ssh-config.example' \
  --exclude '.ftp-deploy-sync-state.json' \
  ./ "${REMOTE}"

echo "Setting uploads directory permissions (if it exists)..."
